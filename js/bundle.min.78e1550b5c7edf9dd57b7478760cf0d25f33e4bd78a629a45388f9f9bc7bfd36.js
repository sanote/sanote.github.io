...
resizeThumb();});window.addEventListener('scroll',function(e){reloadTab();});reloadTab();resizeThumb();});function resizeThumb(){var lists=document.querySelectorAll(".list .thumb .scroll");for(var i=0;i<lists.length;i++){function initImg(box){var to=setInterval(function(){box.style.display="block";var w=box.querySelector('img').naturalWidth;box.querySelector('img').width=w/2;if(w>0){clearInterval(to);box.scrollLeft=(600-box.clientWidth)/2;}},100);}
initImg(lists[i]);}}
